...
export const getLanguageFromFileType = (fileType) => {
  const judge0Lang = JUDGE0_LANGUAGES.find(
    (lang) => lang.extension === fileType
  );
  if (judge0Lang) {
...
export const buildFileTree = (flatFolders, allFiles, localFiles) => {
  ...
  // Assign child folders to parents using parentId
  flatFolders.forEach((f) => {
    if (f.id !== "root" && f.parentId && !assignedFolders.has(f.id)) {
      if (folderMap[f.parentId]) {
  ...
  // Add top-level folders (those with parentId === null and not root)
  flatFolders.forEach((f) => {
    if (f.id !== "root" && !f.parentId && !assignedFolders.has(f.id)) {
  ...
  // Combine server files with local files (localFiles is a Map)
  const combinedFiles = [...allFiles];
  if (localFiles && localFiles instanceof Map) {
  ...
  // Assign files to folders using folderId property
