  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (detailRef.current && !detailRef.current.contains(event.target)) {
        // Check if click was on the profile button
        ...
        <div className="space-y-2">
          {navigationLinks.map((link, index) => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-lg transition-all duration-200 text-left group border border-transparent cursor-pointer ${
                link.action === "logout" 
...
const Navbar = () => {
    ...
    const loadUserData = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/image/user/profile/${user.username}`
        );
  ...
  const handleNavigation = (path, linkId) => {
    console.log(`Navigating to ${path} (Link ID: ${linkId})`);
    router.push(path);
  };
    ...
    <div className="flex justify-end gap-6">
      {!user ? (
        <div className='flex gap-5 mr-5'>
          <Link
            href="/Register"
            className="flex gap-3 bg-white/40 p-2 mt-3 rounded-lg cursor-pointer active:scale-95 transition-transform ease-in-out"
          >
            Sign Up
        ...
        <div className='mr-5'>
          <div className='flex gap-5'>
            ...
            <div className="relative" data-profile-button>
              <button
                className="h-12 w-12 mt-3 rounded-full overflow-hidden border-2 border-white cursor-pointer hover:scale-105 transition-transform duration-200"
